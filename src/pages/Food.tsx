...
import { ArrowLeft, MapPin, Filter, Search, Heart, Navigation, Phone, Star, Clock, ChevronRight, Flame, Zap, DollarSign, Utensils, Info } from "lucide-react";
...
const Food = () => {
  ...
  const getTimeBasedContext = () => {
    ...
    if (hour >= 6 && hour < 11) return {
      title: "Breakfast Spots Open Now",
      icon: "☀️"
    };
    if (hour >= 11 && hour < 14) return {
      title: "Best Lunch Deals Nearby",
      icon: "🍱"
    };
    if (hour >= 14 && hour < 17) return {
      title: "Cafes for a Break",
      icon: "☕"
    };
  ...
  return <div className="min-h-screen bg-background pb-20">
      ...
      <header className="sticky top-0 z-50 w-full bg-card border-b border-border">
            ...
            <SheetContent side="bottom" className="h-[80vh]">
              ...
              <div className="space-y-6 mt-6">
                ...
                <div>
                  ...
                  <Slider value={priceRange} onValueChange={setPriceRange} min={1} max={4} step={1} className="mb-4" />
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {["500m", "1km", "2km", "5km", "Any"].map(dist => <Button key={dist} variant="outline" size="sm">
                        {dist}
                      </Button>)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {["Vegetarian", "Vegan", "Halal", "Gluten-free"].map(pref => <div key={pref} className="flex items-center space-x-2">
                        <Checkbox id={pref} />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {["3.0+", "4.0+", "4.5+"].map(rating => <Button key={rating} variant="outline" size="sm">
                        ⭐ {rating}
                      </Button>)}
                  </div>
                </div>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input placeholder="Search restaurants, cuisines..." className="pl-9" />
          </div>
        </div>
      ...
      <section className="px-4 py-3 border-b border-border">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          {quickFilters.map(filter => <Button key={filter} variant={activeFilters.includes(filter) ? "default" : "outline"} size="sm" onClick={() => toggleFilter(filter)} className="flex-shrink-0">
              {filter}
            </Button>)}
        </div>
      </section>
      ...
      <section className="px-4 py-3 border-b border-border">
        <div className="flex gap-3 overflow-x-auto scrollbar-hide">
          {cuisineTypes.map(cuisine => <Button key={cuisine.id} variant={activeCuisines.includes(cuisine.id) ? "default" : "outline"} size="sm" onClick={() => toggleCuisine(cuisine.id)} className="flex-shrink-0 flex-col h-auto py-2 px-3 gap-1">
      ...
      {/* Featured Restaurant */}
      {restaurants[0] && <section className="px-4 mb-6">
          ...
          <Card className="overflow-hidden cursor-pointer hover:shadow-lg transition-shadow">
            <div className="relative h-48">
              ...
              <Button variant="ghost" size="icon" className="absolute top-3 right-3 bg-white/20 backdrop-blur-sm hover:bg-white/30" onClick={e => {
            e.stopPropagation();
            toggleSaved(restaurants[1].id);
          }}>
                <Heart className={`h-4 w-4 ${savedRestaurants.has(restaurants[1].id) ? "fill-destructive text-destructive" : "text-white"}`} />
              </Button>
      ...
      <section className="px-4">
        ...
        <div className="space-y-3">
          {restaurants.map(restaurant => <Card key={restaurant.id} className="p-4 cursor-pointer hover:shadow-md transition-shadow">
              <div className="flex gap-3">
                ...
                <div className="relative w-24 h-24 rounded-xl overflow-hidden flex-shrink-0">
                  ...
                  {restaurant.isNomiePick && <Badge className="absolute top-1 left-1 text-[9px] px-1.5 py-0 bg-primary">
                      Nomie Pick
                    </Badge>}
                  <Button variant="ghost" size="icon" className="absolute top-1 right-1 h-6 w-6 bg-white/80 hover:bg-white" onClick={e => {
                e.stopPropagation();
                toggleSaved(restaurant.id);
              }}>
                    <Heart className={`h-3 w-3 ${savedRestaurants.has(restaurant.id) ? "fill-destructive text-destructive" : "text-muted-foreground"}`} />
                  </Button>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <Badge variant={restaurant.isOpen ? "success" : "secondary"} className="text-[10px] px-1.5 py-0">
                      {restaurant.isOpen ? <>
                  ...
                  {restaurant.tags[0] && <Badge variant="outline" className="text-[10px] px-1.5 py-0">
                      {restaurant.tags[0]}
                    </Badge>}
                  <div className="flex gap-1 mt-2 flex-wrap">
                    {restaurant.badges.slice(0, 2).map(badge => {
                  const BadgeIcon = badgeIcons[badge];
                  return <div key={badge} className="flex items-center gap-1 text-[10px] text-muted-foreground">
